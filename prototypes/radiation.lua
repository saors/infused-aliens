...
function radiation_util.foreach(table_, fun_)
...
function radiation_util.create_radiation_pictures(opts)
  ...
  local fire_tint = {r=0.1,g=1,b=1,a=0.1}
  ...
  return radiation_util.foreach(retval, function(tab)
    if tab.shift and tab.scale then tab.shift = { tab.shift[1] * tab.scale, tab.shift[2] * tab.scale } end
  end)
end


radiation = table.deepcopy(data.raw["fire"]["fire-flame"])
radiation["name"] = "fire-radiation"
radiation["pictures"] = radiation_util.create_radiation_pictures({ blend_mode = "normal", animation_speed = 1, scale = 0.5})
radiation["damage_per_tick"] = {amount = 50 / 60, type = "fire"}
radiation["maximum_damage_multiplier"] = 1
radiation["maximum_spread_count"] = 0
radiation["initial_lifetime"] = 1200
